  ...
  return (
    <div className="glass-card bg-white/80 border border-white/60 rounded-[16px] p-6 md:p-8 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
      <div className="flex items-center gap-4">
        {icon && (
          <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gold/10 text-gold flex items-center justify-center shadow-sm">
            {icon}
          </div>
        )}
        <div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif font-normal text-charcoal">{title}</h1>
          {subtitle && <p className="text-charcoal/70 mt-1 text-sm md:text-base">{subtitle}</p>}
        </div>
      </div>
      {rightSlot && (
