...
export const OverviewStatCard: React.FC<OverviewStatCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`p-2.5 rounded-full w-11 h-11 flex items-center justify-center transition-colors shrink-0 mb-3 ${iconClassName} group-hover:bg-gold group-hover:text-white ${highlight ? 'bg-gold/15' : ''}`}
      >
        {icon}
      </div>
      <p className="flex min-h-11 items-center justify-center text-center px-10 text-[18px] font-semibold text-charcoal leading-snug">
        {label}
      </p>
      <div
        className={`${valueSizeClasses[valueSize]} mb-2 text-center px-10 normal-case ${highlight ? 'text-gold' : 'text-charcoal'} ${valueClassName}`}
      >
        {value}
      </div>
      {description ? (
