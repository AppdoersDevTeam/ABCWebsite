...
> Order admin and member menus for best UI/UX, fix dashboard colours to match the public website, and repair overlapping non-responsive admin overview cards.

**Changes**

* Replaced alphabetical left-menu sorting with intentional UX order and section dividers for admin and member portals.
* Unified portal nav and overview card icon colours to the public site gold/charcoal language (removed rainbow icon chips).
* Rebuilt OverviewStatCard layout: left-aligned icon and label, readable sans-serif charcoal stats, full-width responsive grids.
* Admin Overview cards no longer shrink to 72% width in a cramped five-column layout; approval actions use gold/charcoal/red styling aligned with the public site.
* Admin and member page headers use charcoal Kaushan Script titles with gold accents, matching public section headings.
...
* Lint: not run or failed
* Build: passed
* Notes: Portal nav unit tests plus npm test and npm run validate. vite build. Typecheck still fails on pre-existing app errors.
...
> decrease another 2px. push it.

**Changes**

* Each Admin Overview stat card counter is another 2px smaller.
...
## CHG-2026-2109-041 — Overview card counters 3px smaller
...
> Decrease the font of each cards counter number in 3px. push live

**Changes**

* Each Admin Overview stat card counter is 3px smaller.
...
## CHG-2026-2109-040 — Overview cards 15% wider
...
> Every Admin Overview stat card  increase the width size in  15% more. Height and inner layout are unchanged.. push it.

**Changes**

* Every Admin Overview stat card is 15% wider than the previous live width. Height and inner layout are unchanged.
...
> Every Admin Overview stat card  narrow it in  25% less. Height and inner layout are unchanged. push it

**Changes**

* Every Admin Overview stat card is 25% narrower than the previous live width. Height and inner layout are unchanged.
...
## CHG-2026-2109-035 — Overview cards 25% narrower
...
> Every Admin Overview stat card is 25% narrower. Height and inner layout are unchanged. push it

**Changes**

* Every Admin Overview stat card is 25% narrower than the previous full card width. Height and inner layout are unchanged.
...
> make a correction on the Users cards, Bring all icons righ under the Card Icon push all UP. push alive. All the Count numbers increase 4px. push live

**Changes**

* On the Users card, Approved, Pending, and Not linked now sit directly under the card icon and are pushed up.
* All Overview count numbers are 4px larger.
...
> For all cards increase the size of the arrow as per image in 35%, make it bold, also for each card description align it in the middle of the line to be in the middle of the aligment of the ICON on left. push to live.

**Changes**

* Overview card arrows are 35% larger, gold, and bolder.
* Each card title, number, and description now sit in line with the left icon, with the description vertically centred on its row.
...
> On Overview The card for user Approval Requests, decrease the hight of users line as well as the buttons on the right side proportionally to feet better in the page, also decrease the space btween the users registers.

**Changes**

* User Approval Requests rows are shorter, with smaller right-side buttons and less space between each person.
